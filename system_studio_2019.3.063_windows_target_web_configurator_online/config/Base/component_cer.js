...
new function () 
{
    ...
    var ns_prc       = load("component_processor_cer.js");

    var ns = this;
    //###############################################################
    // ComponentCER constructor
    //###############################################################
    //###############################################################
    ...
    this.Create = function (_in)
    {
        ...
        Log("Crt r_info.Id=" +r_info.Id);
        if(!r_info || !r_info.Id || !r_info.Id())
        {
            Log(Log.l_error, "Attempt to create Crt component with undefined Id - input info isn't defined or doesn't have Id or Id() is empty");
            return null;
        }
    ...
    {
        Log("Creating crt component");

        if(!_in.Processor)
            _in.Processor = ns_prc.ProcessorCer();

        var cmp = ns_base_cmp.Component(_in);

        cmp.Log = log_helper("ComponentCER id = " + cmp.Id() + ": ");

        cmp.Type("cer_component");
