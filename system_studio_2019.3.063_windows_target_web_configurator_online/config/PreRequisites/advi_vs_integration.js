...
new function()
{
    ...
    this.Id = function()
    {
        return "advi_vs_integration_prerequisites";
    }
    ...
    this.CheckStage_Second = function (collector, product) {
        ...
        var vs2012 = (ns_psxe.ComponentByAlias(product, "advi_vs2012-integration"));
        var vs2013 = (ns_psxe.ComponentByAlias(product, "advi_vs2013-integration"));
        var vs2015 = (ns_psxe.ComponentByAlias(product, "advi_vs2015-integration"));
        var vs2017 = (ns_psxe.ComponentByAlias(product, "advi_vs2017-integration"));

        var vs2012Exists = (ns_psxe.IsComponentExistsAndSelectedByAlias(product, "advi_vs2012-integration"));
        var vs2013Exists = (ns_psxe.IsComponentExistsAndSelectedByAlias(product, "advi_vs2013-integration"));
        var vs2015Exists = (ns_psxe.IsComponentExistsAndSelectedByAlias(product, "advi_vs2015-integration"));
        var vs2017Exists = (ns_psxe.IsComponentExistsAndSelectedByAlias(product, "advi_vs2017-integration"));

        description = (ns_inst.Installer.Silent()) ? "[advi_older_integrated_into_vs_description_silent]" : "[advi_older_integrated_into_vs_description]";

        // Check for Inspector
        if (!WI.ClientsInstalledComponent("{FA2451B0-C22F-4E33-80AE-F67A29EE7E02}"))
        {
            description = (ns_inst.Installer.Silent()) ? "[advi_older_integrated_into_vs_description_silent_tcar_custom]" : "[advi_older_integrated_into_vs_description_tcar_custom]";
        }
        ...
        if((vs2012Exists || vs2013Exists || vs2015Exists || vs2017Exists) && product.InstallMode() == product.install_mode_t.install)
        {
            if( vs2012Exists && (vs2012.Upgrade().OlderExists() || vs2012.Upgrade().NewerExists())
                ...
                collector.WarningExt(StringList.Format("[advi_older_integrated_into_vs_title]"), StringList.Format(description));
        }
