...
new function()
{
    ...
    this.Id = function()
    {
        return "amplxe_common_prerequisites";
    }
    ...
    this.CheckStage_Second = function (collector, product) {
        ...
        Log("amplxe Product name = " + product.Name());

        var tmp_product = product;
        var original_product = product;

        if(product.Id().indexOf("amplxe") == -1)
        {
            //incoming product isn't an amplxe
            // try to find it in incoming product

            Log("product isn't an amplxe");
            product.FilterFeaturesRecursive(function(ftr)
            {
                if(ftr.Type() == "product" && ftr.Id().indexOf("amplxe") == 0)
                {
                    tmp_product = ftr;
                    return true;
                }
        ...
        var image = StringList.Format("[$VTUNE_AMPLIFIER_2019_DIR]");
        var running = app_is_running(image);

        var separator = (ns_inst.Installer.Silent()) ? "; " : "\\par\\pard\\li280";
        var description = (ns_inst.Installer.Silent()) ? "[amplxe_app_running_description_silent]" : "[amplxe_app_running_description]";

        if(running.length)
            collector.CriticalExt(StringList.Format("[amplxe_app_running_title]"),
        ...
        if(original_product.InstallMode() == product.install_mode_t.install)
        {
            ...
            if(installed)
            {
                ...
                for(var key in feature_set)
                {
                    ...
                    if (current_feature.Id().match(/amplxe_toplevel/i))
                    {
                ...
                if(toplevel_feature && toplevel_feature.Upgrade().OlderExists())
                    collector.WarningExt(StringList.Format("[amplxe_older_installed_title]"),
                                         StringList.Format("[amplxe_older_installed_description]"));

                else if(toplevel_feature && toplevel_feature.Upgrade().NewerExists())
                    collector.WarningExt(StringList.Format("[amplxe_newer_installed_title]"),
                                         StringList.Format("[amplxe_newer_installed_description]"));
            }
        }
